# When to Use PelagoDB (And When Not To)
...
Important note:
- Product capabilities evolve by version, edition, and deployment model.
- Treat this as architecture-fit guidance, not a licensing/featuresheet replacement.
...
- keep updates/mutations safe and conflict-aware,
- keep global relationships queryable,
- keep the system writable under partition,
- and surface conflicts explicitly instead of silently corrupting state.
...
- or the easiest zero-ops graph database for all workloads.
...
| Data-shape governance | Strong schema contracts and validation | Flexible model with constraints/indexes as needed | Schema-driven GraphQL model is common | Schema capabilities depend on backend/config choices |
| Query surfaces | CEL + PQL + gRPC | Cypher ecosystem depth | GraphQL/DQL workflows | Gremlin/TinkerPop ecosystem |
| Multi-site model | Explicit CDC pull replication + ownership controls | Varies by edition/deployment choices | Distributed model with different replication semantics | Depends heavily on backend topology |
...
| Ecosystem/tooling maturity | Growing | Very mature | Mature in distributed GraphQL use cases | Mature in custom stack deployments |
...
- Requires intentional data modeling and capacity planning to get strong p99 behavior.
- Under partition, conflicting new-data creation is resolved by LWW and may require downstream reconciliation/dead-letter handling.
...
- Is our team already heavily standardized on Cypher or Gremlin workflows?
- Do we require strong consistency behavior for all conflict classes without reconciliation work?
...
4. Verify that create-time conflict handling and reconciliation flows are acceptable to your product team.
