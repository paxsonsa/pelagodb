...
- `PELAGO_REPLICATION_SCOPES` (optional CSV: `db/ns,db2/ns2`; overrides database/namespace)
...
- `PELAGO_MTLS_ENABLED` (default: `false`)
- `PELAGO_MTLS_SUBJECT_HEADER` (default: `x-mtls-subject`)
- `PELAGO_MTLS_SUBJECTS` (`subject=principal` CSV)
- `PELAGO_MTLS_FINGERPRINTS` (`sha256:fingerprint=principal` CSV)
- `PELAGO_MTLS_DEFAULT_ROLE` (default: `service`)
- `PELAGO_TLS_CERT` (PEM cert path)
- `PELAGO_TLS_KEY` (PEM key path)
- `PELAGO_TLS_CA` (PEM CA bundle path, required for client cert validation)
- `PELAGO_TLS_CLIENT_AUTH` (`none`, `request`, or `require`; default: `none`)
