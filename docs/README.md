# PelagoDB Documentation

## Recommended Paths

### Application Developer Path
- `docs/01-getting-started.md`
- `docs/14-data-modeling-and-scaling.md`
- `docs/16-when-to-use-pelagodb.md`
- `docs/02-cli-reference.md`
- `docs/03-api-grpc.md`
- `docs/17-watch-and-streaming.md`
- `docs/10-client-sdks.md`

### Operations and Platform Path
- `docs/04-administration.md`
- `docs/05-security-and-access.md`
- `docs/06-replication-and-multi-site.md`
- `docs/13-centralized-replication-and-scaling.md`
- `docs/15-architecture-and-design.md`
- `docs/17-watch-and-streaming.md`
- `docs/09-operations-playbook.md`
- `docs/11-troubleshooting.md`
- `deploy/k8s/README.md`
- `deploy/k8s/CHECKLIST.md`

### Demo/Presentation Path
- `docs/08-presentation-runbook.md`
- `datasets/README.md`

## Reference Index

## Start Here
- `docs/01-getting-started.md`
- `docs/14-data-modeling-and-scaling.md`
- `docs/16-when-to-use-pelagodb.md`
- `docs/02-cli-reference.md`

## API and Integration
- `docs/03-api-grpc.md`
- `docs/17-watch-and-streaming.md`
- `clients/README.md`

## Operations and Administration
- `docs/04-administration.md`
- `docs/05-security-and-access.md`
- `docs/06-replication-and-multi-site.md`
- `docs/07-production-readiness.md`
- `docs/09-operations-playbook.md`
- `docs/13-centralized-replication-and-scaling.md`
- `docs/15-architecture-and-design.md`
- `docker-compose.multisite.yml`
- `deploy/k8s/README.md`

## Demo and Example Data
- `datasets/README.md`
- `docs/08-presentation-runbook.md`

## SDK Integration
- `docs/10-client-sdks.md`

## Troubleshooting
- `docs/11-troubleshooting.md`

## Hosted Docs
- `docs/12-server-docs-site.md`

## Source of Truth
- Protocol/API contract: `proto/pelago.proto`
- Server runtime behavior: `crates/pelago-server/src/main.rs`
- Storage semantics and enforcement: `crates/pelago-storage/src`
