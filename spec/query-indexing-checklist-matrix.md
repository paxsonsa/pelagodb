...
| Native `OR` optimizer | Reduce large union query latency | Posting lists + batched node fetch path | Server-side union with batched fetches and reduced per-node round trips | TODO | `OR` p99 approaches fan-out p99 on representative workloads |
...
| Pagination semantics | Stable large-result handling | Index-key cursors | Keyset pagination (not offset) | IN_PROGRESS | Deep-page latency remains bounded and monotonic |
| Stateless query tier scale-out | Support horizontal query-node scaling without sticky state | External request context + cursor-only continuation tokens | Any query node can serve any page/request | TODO | N query nodes behind LB show linear-ish throughput gain with no correctness regressions |
